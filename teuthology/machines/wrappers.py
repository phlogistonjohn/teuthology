...
def update_nodes(
    nodes, reset_os=False, *, machine_pool: MachinePool | None = None
) -> None:
    pool = auto_pool(pool=machine_pool)
    update_nodes_fn = getattr(pool, 'update_nodes', None)
    if update_nodes_fn is None:
        raise NotImplementedError("pool does not support update_nodes")
    return update_nodes_fn(nodes, reset_os=reset_os)
